...
| Production supervision | Production child specs constructed with/without DB; the direct stdin port, server, or Rust port may fail | EOF causes one clean exit; stdin error exits visibly nonzero; child startup failure fails application startup. One `McpServer` directly owns the native fd 0 port and is the sole input owner. Its abnormal termination restarts the declared child once under supervisor policy; `PortManager` crash/restart remains owned by `PortProtocol.tla` and must not create a second input owner | Coordinate with `bd-pia3.1`; do not alter its startup closure model | New `mcp/test/application_lifecycle_test.exs` with injected IO/child specs and existing fake ports; assert child counts, restart count, exit status; package smoke remains `bd-pia3` |
