...
VARIABLES phase, owner, child, active, observed, pending, terminationRequested,
          ...
          acceptedAttempt, report, logBytes
...
Init ==
  ...
  /\ acceptedAttempt = 0 /\ report = "none" /\ logBytes = 0
...
ClientRequest ==
  ...
  /\ UNCHANGED <<phase, owner, child, active, observed, terminationRequested,
                 attempt, acknowledgedAttempt, workerStore, acceptedAttempt,
                 report, logBytes>>
...
Launch ==
  ...
  /\ \E result \in LaunchResults:
       ...
       ELSE /\ phase' = "failed" /\ child' = "none" /\ active' = 0
            ...
            /\ attempt' = attempt + 1
            /\ acknowledgedAttempt' = IF AcknowledgesFailedLaunch
                                        THEN attempt + 1 ELSE acknowledgedAttempt
            /\ UNCHANGED acceptedAttempt
...
(* stdout/stderr may arrive independently of lifecycle transitions.  This
   abstract retained-byte counter models bounded in-memory/log retention, not
   a concrete rotation implementation. *)
OutputChunk ==
  /\ owner /\ child = "live" /\ phase \in {"running", "cancelling"}
  /\ logBytes' = IF OutputBounded THEN LogAdd(logBytes) ELSE logBytes + 1
  /\ UNCHANGED <<phase, owner, child, active, observed, pending,
...
Next == ClientRequest \/ CoalesceRequest \/ Launch \/ Finish \/ Cancel \/
        ObserveTermination \/ OwnerDeath \/ OutputChunk
...
TypeOK ==
  ...
  /\ acceptedAttempt \in 0..MaxRequests
