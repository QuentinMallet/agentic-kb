...
`db::suppress_close_checkpoint`, `db::open_unchecked_for_test`, and
...
`checkpoint-suppressor` means `db::suppress_close_checkpoint(...)` — a
`SQLITE_OPEN_READ_ONLY` handle held alongside such a writer for exactly the
lifetime of its connection, so SQLite does not run its close-time checkpoint
inside the lock window; it never writes and never outlives the lock hold. `init` means `db::open_or_init(...)`. `scratch` means
...
| `src/components/db.rs` | contract definitions: read-only + write-under-lock + reopen-under-lock + checkpoint-suppressor + scratch + checkpoint + init + test-fixture + test-fixture-init | Defines `open_live_for_checkpoint`, `open_ro`, `open_rw`, `open_rw_existing`, `suppress_close_checkpoint`, `open_scratch`, `open_or_init`, `open_unchecked_for_test`, and `test_db`; the ratchet permanently forbids the removed legacy opener and separately confines `open_unchecked_for_test` to test modules. |
