...
ADR-1 divides repository access among nine named openers:
`db::open_ro`, `db::open_rw`, `db::open_rw_existing`, `db::open_scratch`,
`db::open_or_init`, `db::open_live_for_checkpoint`,
`db::defer_checkpoints`, `db::open_unchecked_for_test`, and
...
guard. `reopen-under-lock` means `db::open_rw_existing(paths, &lock)` — the
same lock proof without the DDL and stamp, for a batched writer that must
reopen the live pathname on every batch so a `rebuild` swap is picked up.
`checkpoint-deferral` means `db::defer_checkpoints(...)` — a
`SQLITE_OPEN_READ_ONLY` handle held alongside such a writer for exactly the
lifetime of its connection, plus `wal_autocheckpoint=0` on the writer, so
neither SQLite's close-time checkpoint nor its automatic one runs inside the
lock window; the handle never writes and never outlives the lock hold. What makes it
genuinely non-writing is not the SQL-level flag on its own: `unixShmMap` maps
`-shm` `PROT_READ` only for a read-only file handle, so this handle takes its
shared lock with `F_RDLCK` and never records a wal-index read mark. Reaching
for `open_ro` plus `query_only=ON` instead would not do — that opener is
`O_RDWR` by design, and its close would checkpoint. `init` means `db::open_or_init(...)`. `scratch` means
...
| `src/commands/reembed.rs` | mixed: read-only + write-under-lock + reopen-under-lock + checkpoint-deferral + init | Read opens in `run_reembed_with_hooks` and `confirm_embedded_ids_are_live`; on an uninitialized repository, `--dry-run` maps to an empty report, while non-dry-run calls `open_or_init` itself (not the caller) before retrying, mirroring `stale_check.rs`'s `heal_relocations`; `preflight_reembed_schema` takes the lock once for `open_rw` (schema, stamp, and the `embed_text_mode` row) so no batch pays for them; each batch in `write_batches` then reopens the live pathname with `open_rw_existing` under a fresh lock hold, alongside a `defer_checkpoints` guard dropped with it, and `drain_wal` takes the lock once every `REEMBED_DRAIN_EVERY_BATCHES` batches and once after the last one to backfill and truncate the WAL those batches deliberately left; at CLI dispatch, `KbCmd::mutates` classifies `--dry-run` invocations as non-mutating; test setup uses `open_or_init` extensively; `check_embed_mode_vintage` stays a separate contract row below. |
...
| `src/components/db.rs` | contract definitions: read-only + write-under-lock + reopen-under-lock + checkpoint-deferral + scratch + checkpoint + init + test-fixture + test-fixture-init | Defines `open_live_for_checkpoint`, `open_ro`, `open_rw`, `open_rw_existing`, `defer_checkpoints`, `open_scratch`, `open_or_init`, `open_unchecked_for_test`, and `test_db`; the ratchet permanently forbids the removed legacy opener and separately confines `open_unchecked_for_test` to test modules. |
| `src/components/kb_core.rs` | write-under-lock + test-fixture + test-fixture-init | Production add uses `open_rw` and `cursor::append_and_apply_writer_events`; most unit-test inspection uses `open_unchecked_for_test`; the shared `setup()` fixture initializes a fresh repository with `test_db`. |
...
| reembed checkpoint policy | stated policy | A `reembed` batch defers both of SQLite's own checkpoints: the close-time one, and the automatic one (`wal_autocheckpoint=0`) that would otherwise fire from inside a COMMIT once the WAL passed 1000 pages, putting a checkpoint's fsyncs back into a lock window on one batch in every few dozen. Nothing else then bounds the WAL, so `write_batches` drains it every `REEMBED_DRAIN_EVERY_BATCHES` (8) batches and once more after the last, each time under its own hold of `paths.lock` and between batch windows rather than inside one. The periodic drains use `wal_checkpoint(PASSIVE)`, which backfills without truncating so the next commit reuses the file in place; `TRUNCATE` there measured as roughly doubling the following batch, since it forces a fresh header into freshly allocated blocks. Only the run's last drain truncates, leaving the database at rest self-contained. That caps the WAL near 0.75 MiB, well under SQLite's own default bound. A drain's flock hold is a lock window like any batch's and is held to the same 50 ms budget by `test_reembed_batch_lock_hold_budget`; it measured 20.207 ms and 20.668 ms at `load1` 4.9, and the interval is sized to keep it there, since a drain's cost grows with the number of frames it backfills. A drain that cannot run warns on stderr and costs no committed write. |
| reembed deferred checkpoint | swap invariant, unchanged | A `reembed` batch defers SQLite's close-time checkpoint, so between batches the live database may carry a `-wal` with committed but unbackfilled frames. `db::open_ro_peer` reads a peer with
`immutable=1` and so ignores a log left undrained by a crash or a warned drain
failure, omitting that run's rows from federated search until the peer's next
write or rebuild; see `docs/src/peer-graph-cli.md`. That is durable (the commit fsynced the WAL under `synchronous=FULL`, and recovery replays it) and it does not weaken the swap: the suppressing handle is opened and dropped inside one hold of `paths.lock`, so no connection is open on the live inode when the lock is released, and the rename is still preceded by `checkpoint_live_db` draining that WAL and `verify_live_wal_drained` proving it zero-length. `drain_wal` restores a self-contained database at the end of the run, and `test_no_checkpoint_runs_inside_a_batch_lock_window` and `test_reembed_drains_the_wal_before_returning` pin both halves. |
