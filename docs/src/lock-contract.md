...
| `src/commands.rs` | init | `EntryPoint::run` calls `db::open_or_init` only when `KbCmd::mutates` classifies the selected leaf command as mutating; per-subcommand classification lives in `KbCmd::mutates`. |
