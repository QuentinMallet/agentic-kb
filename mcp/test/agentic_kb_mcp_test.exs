...
defmodule AgenticKbMcpTest do
  ...
  describe "tools/call request path (B1)" do
    ...
    test "audit run, verdict recording, and report compose through tools/call" do
      ...
      # "audit-perm" is a fixture-only id fake_port.sh special-cases: a false
      # verdict against it always refuses with permanent_guard, so the
      # rendered refusal below is the only place the Elixir side of that
      # error path (the generic %{"type" => "error"} -> text_error clause)
      ...
      assert perm_text =~ "audit-perm"
      assert perm_text =~ "permanent"
