defmodule AgenticKbMcp.OptionBAccessTest do
  ...
  test "every advertised tool is available without package authorization" do
  ...
  test "formerly guarded expire and audit operations dispatch anonymously" do
    for {name, arguments} <-
  ...
  test "client caller_id is rejected by every closed tool schema" do
    for {name, arguments} <- @tool_arguments do
      response = call_tool(name, Map.put(arguments, "caller_id", "client-supplied"))
