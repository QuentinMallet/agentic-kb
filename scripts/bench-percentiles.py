...
def main():
    ...
    artifact={"meta":{"date":datetime.date.today().isoformat(),"branch":git("branch","--show-current"),"commit":git("rev-parse","--short","HEAD"),"embedder":"BenchEmbedder deterministic 384-dim pool","seed":42,"sample_size":{"10000":a.sample_size,"100000":a.large_sample_size},"note":"CLI end-to-end; p95<500ms@10k cold is authoritative. 100k uses reduced sample_size; Criterion 100k groups use SamplingMode::Flat.","mode":a.mode,"actual_elapsed_seconds":a.elapsed_seconds},"results":results}
