...
// The log reader's fast path (`events::ends_on_intact_span`) only looks back
// `TAIL_WINDOW` (64 KiB) from EOF; a span that doesn't fit inside that window
// forces a full log scan on every write against the fixture (O(log size)
// instead of O(bytes appended)), which is exactly the cost this fixture must
// not add to the write-lane benchmark it feeds. At ~465 bytes/event, 128
// events per span is ~60 KiB — inside the window with headroom for longer
// entries. Keep this well under `events::TAIL_WINDOW` if either constant
// changes; see tests::seeded_fixture_final_span_fits_the_reader_tail_window.
const SEED_BATCH_EVENTS: usize = 128;
...
pub fn seed_db(conn: &Connection, emb: &BenchEmbedder, n: usize, seed: u64) -> anyhow::Result<()> {
    let mut rng = StdRng::seed_from_u64(seed);
    for i in 0..n {
        let [upsert, evidence] = fixture_events(i, &mut rng);
        db::apply_event(conn, emb, &upsert)?;
        db::apply_event(conn, emb, &evidence)?;
    }
    Ok(())
}
...
/// `SEED_BATCH_EVENTS` limit memory and amortize the convergence gate/fsync
/// while leaving every completed chunk as a framed, converged log prefix
/// small enough for the log reader's tail-window fast path (see the comment
/// on `SEED_BATCH_EVENTS`).
pub fn seed_fixture(
    lock: &Lock,
    conn: &Connection,
    paths: &config::Paths,
...
pub fn logical_checksum(conn: &Connection) -> anyhow::Result<String> {
    ...
    for sql in [
        "SELECT id||'|'||path||'|'||summary||'|'||content||'|'||tags||'|'||evidence_status FROM entries ORDER BY id",
