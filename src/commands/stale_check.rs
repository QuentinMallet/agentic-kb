...
fn heal_relocations(
    ...
    let repo_root = paths.db.parent().and_then(Path::parent).and_then(Path::parent)
    ...
    for r in report.relocation.iter_mut() {
        ...
        let Some(evidence): Option<Evidence> = conn
            .query_row(
                ...
                |row| Ok(Evidence {
                    id: row.get(0)?, entry_id: row.get(1)?, kind: row.get(2)?,
                    citation_path: row.get(3)?, citation_sha: row.get(4)?,
                    citation_hash: row.get(5)?, citation_excerpt: row.get(6)?,
                    derived_from: row.get(7)?, recorded_at: row.get(8)?,
                }),
            )
            .optional()?
        else {
...
mod tests {
    ...
    fn heal_relocations_discards_missing_and_nonconforming_plans() {
        ...
        assert!(!report.relocation[1].healed, "a stale planned destination is discarded");
        ...
        assert_eq!(
            events.events.len(),
            0,
            "neither stale plan is appended"
        );
    ...
    fn concurrent_heal_writers_discard_second_stale_destination() {
        ...
        conn.execute(
            "INSERT INTO evidence(id,entry_id,kind,citation_path,citation_hash,citation_excerpt)
             VALUES('ev1','e1','code','a.rs:0-10','sha256:wrong',?1)",
            [&excerpt],
        ).unwrap();
        drop(conn);
            ...
            joins.push(std::thread::spawn(move || {
                let mut report = StaleCheckReport {
                    relocation: vec![RelocationEntry {
                        entry_id: "e1".into(), evidence_id: "ev1".into(),
                        status: VerificationStatus::Relocated,
                        old_path: "a.rs:0-10".into(), new_path: Some(destination.into()),
                        reason: None, healed: false,
                    }],
                    ..Default::default()
                };
                barrier.wait();
        ...
        let path: String = conn.query_row(
            "SELECT citation_path FROM evidence WHERE id='ev1'", [], |r| r.get(0)
        ).unwrap();
    ...
    fn heal_relocations_errors_on_undecodable_citation_path_column() {
        ...
        let mut report = StaleCheckReport {
            relocation: vec![RelocationEntry {
                entry_id: "e1".into(), evidence_id: "ev1".into(),
                status: VerificationStatus::Relocated, old_path: "a.rs".into(),
                new_path: Some("b.rs".into()), reason: None, healed: false,
            }],
            ..Default::default()
        };
        let err = heal_relocations(&paths, &mut report, RelocationPolicy::FileThenRepo)
            .unwrap_err();
