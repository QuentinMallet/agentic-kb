...
pub fn add_locked(
    ...
    let mut resolved_hashes: HashMap<(FileIdentity, Option<(usize, usize)>), String> = HashMap::new();
    ...
    for evidence in &mut args.evidence_rows {
        ...
        if !hash_missing {
            let supplied = evidence.get("citation_hash").and_then(Value::as_str).unwrap();
        ...
        object.insert("citation_hash".to_string(), Value::String(citation_hash.clone()));
        ...
        resolved_citations.push(ResolvedCitation {
            citation_path, absolute_path, identity, hash: citation_hash,
        });
    }
    ...
    let mut verified_identities: HashMap<(FileIdentity, Option<(usize, usize)>), String> = HashMap::new();
    for resolved in &resolved_citations {
        ...
        if current_identity != resolved.identity {
            anyhow::bail!("citation changed before append: {:?}", resolved.citation_path);
        }
...
mod tests {
    ...
    fn test_kb_core_add_catches_mutation_between_resolution_and_append() {
        ...
        let err = add(
            &paths,
            &NoopEmbedder,
            evidence_args(vec![serde_json::json!({"kind":"code","citation_path":"cited.txt"})]),
        ).unwrap_err();
        assert!(err.to_string().contains("citation changed before append"), "{err}");
        assert_eq!(fs::read(&paths.events).unwrap_or_default(), before);
    }
