...
pub fn compute_citation_fields(
    ...
    compute_citation_fields_with_verifier(repo_root, rel_path, range, |file, ev, root| {
        Ok(verify_evidence_from(file, ev, root, RelocationPolicy::Never))
    })
}
    ...
    F: FnOnce(&CitationFields) -> Result<T>,
{
    compute_citation_fields_with_verifier_and_emit(
        ...
        rel_path,
        range,
        |file, ev, root| Ok(verify_evidence_from(file, ev, root, RelocationPolicy::Never)),
        emit,
    )
}
...
fn ensure_path_still_names_file(file: &File, path: &Path) -> Result<()> {
    ...
    let current = std::fs::metadata(path)
        .map_err(|_| anyhow!("citation_path changed during cite"))?;
    #[cfg(unix)]
    {
    ...
    #[cfg(not(unix))]
    {
        ...
        if retained.len() != current.len()
            || retained.modified().ok() != current.modified().ok()
        {
            bail!("citation_path changed during cite");
        }
    }
