...
mod tests {
    ...
    #[test]
    fn test_kb_core_add_rejects_non_finite_embedding() {
        struct NonFiniteEmbedder;
        impl embedder::Embedder for NonFiniteEmbedder {
        ...
        let (_dir, paths) = setup();
        let mut args = evidence_args(vec![]);
        args.id = "non-finite-write".to_string();
        args.evidence_status = "missing".to_string();

        let error = add(&paths, &NonFiniteEmbedder, args).unwrap_err();
        assert!(error
            .to_string()
            .contains("embedder returned a non-finite component"));

        let conn = Connection::open(&paths.db).unwrap();
        let entry_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM entries WHERE id='non-finite-write'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        let embedding_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM entries_emb", [], |row| row.get(0))
            .unwrap();
        assert_eq!(entry_count, 0, "failed upsert transaction must roll back");
        assert_eq!(embedding_count, 0, "must reject, never encode zeros");
    }
